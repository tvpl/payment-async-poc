...
| O consumidor Kafka (`PaymentResponseConsumer`) não deve confirmar offset se a escrita do status falhar | propagar a exceção (não engolir) | O caminho HTTP e o caminho Kafka têm requisitos opostos: o HTTP deve responder ao cliente; o consumer deve reprocessar. Engolir no consumer perderia o resultado do pagamento silenciosamente. | y |
