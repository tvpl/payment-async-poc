...
**Gate evidence:** Publiquei `payment-contracts` (`publishAllToLocalBuildRepository`) e `feature-control` (`publishMavenPublicationToLocalBuildRepository`) nos repositórios Maven locais, construí as quatro imagens Docker reais (`payment-api`, `payment-sbus`, `payment-core-mock`, `async-redis-service`) a partir desses repositórios publicados — cada `Dockerfile` só recebe `gradle/`, `gradlew`, `settings.gradle`, `build.gradle`, `src/main/` da própria fronteira mais os dois `additional_contexts` com os repositórios publicados, nunca fonte irmã — e subi as quatro no `/sandbox` via seus próprios `compose.yaml`, ligados pela rede externa `payment-sandbox` (AD-003).

`scripts/e2e/check_no_composite_build.py` prova estruturalmente que nenhum dos quatro `settings.gradle`/`build.gradle` reais usa `includeBuild`, `project(':payment-*'` ou `project(':feature-control'` nem referencia `src/` de fronteira irmã; combinado com `scripts/artifacts/verify-artifact-only.sh` (mecanismo genérico já existente, T1/T6: GAV publicado resolve, GAV ausente falha) isso fecha MIG-05.

Rodar a prova ao vivo achou e corrigiu dois bugs reais de wiring que só apareciam com os containers de pé (não com testes unitários/IT que usam Testcontainers isolado): `payment-api/compose.yaml` não declarava `JWT_SIGNATURE_SECRET` (o perfil `dev`, ativo por padrão, falha no boot sem ele — `application-dev.yml:14`) e `payment-sbus/compose.yaml` declarava `POSTGRES_JDBC_URL` (nome de variável do perfil `prod`) mas roda em `dev` por padrão, cujo `application.yml:74` usa `POSTGRES_HOST`/`POSTGRES_PORT`/`POSTGRES_DB` — nunca definidos, caindo no default `localhost`, que não existe dentro do container. Adicionei as variáveis faltantes (`JWT_SIGNATURE_SECRET`, `SBUS_DEV_JWT_SECRET`, `POSTGRES_HOST/PORT/DB`) a ambos os `compose.yaml` e `.env.example`, documentando por que cada uma só se aplica ao perfil `dev`.

Também achei que o gate de equivalência (`scripts/equivalence/baseline-manifest.json`) não era regenerado desde a Fase 2: 44 arquivos relocados nas Fases 7-8 apareciam como "perdidos" (todos confirmados como puro move por hash idêntico ou por conteúdo relocado+modificado, nunca por perda real — script de verificação descartado após uso) e havia colisão de chave em `topics` porque `common/` (legado, congelado por MIG-02) e `payment-contracts` definem os mesmos tópicos simultaneamente. Corrigi `scripts/equivalence/equivalence.py`: excluí as raízes transicionais congeladas (`common`, `api-service`, `sbus-service`, `core-mock` — remoção é a própria T59) da varredura, e atualizei o glob de `schemas` para `payment-contracts/schemas/*.avsc` (local atual, `contract-model/build.gradle:16-18`) em vez do antigo `src/main/avro/`. `equivalence: PASS (389 entries)`, com teste novo (`test_legacy_transitional_roots_are_excluded`) provando a exclusão.

Com a infra de pé, `scripts/verify-workspace.sh all` roda as seis etapas em sequência e todas passam: `equivalence`, `no-composite-build`, `artifact-only-fixture`, `e2e-payment` (`scripts/smoke.sh`: POST `/payment-simulations` → `200 COMPLETED` com fluxo Kafka→SBUS→core-mock completo), `e2e-async-redis` (`scripts/e2e/async_redis_smoke.sh`, novo: POST `/jobs` → `200 COMPLETED` com fluxo Redis Streams→worker→BRPOP completo), `hygiene` (`git diff --check`). Os dois fluxos E2E satisfazem "≥2 E2E completos".

| Done-when / requisito | Evidência `file:line` e comando | Resultado definido | Coberto? |
| --- | --- | --- | --- |
| MIG-05: build source-level compartilhado é detectado (ausente) | `scripts/e2e/check_no_composite_build.py` execução real: `no-composite-build: PASS (4 boundaries checked)`; `test_check_no_composite_build.py:15-19` roda contra as 4 fronteiras reais, não fixture | nenhum `includeBuild`/`project(':payment-*'`/`project(':feature-control'` nos 4 consumidores reais | ✅ |
| MIG-04/MIG-05: artefato publicado resolve, ausente falha (mecanismo) | `scripts/artifacts/verify-artifact-only.sh` execução real dentro de `verify-workspace.sh`: `artifact-only-consumer: PASS (published GAV resolves; missing GAV fails)` | consumer fixture compila com repo publicado e falha sem ele | ✅ |
| ORG-08: inventário não perde item válido | `scripts/equivalence/equivalence.py verify` execução real: `equivalence: PASS (389 entries)`; `test_equivalence.py:29-32` (`test_current_repository_matches_baseline`) contra a árvore real | zero `missing` entry após reconciliação; todo item das Fases 7-8 rastreado como rename/adição legítima | ✅ |
| Done-when: ≥2 E2E completos passam pelos Composes independentes | `scripts/smoke.sh` execução real: `HTTP 200`, `status:"COMPLETED"`; `scripts/e2e/async_redis_smoke.sh` execução real: `HTTP 200`, `status:"COMPLETED"` | dois fluxos completos ponta a ponta via containers reais na rede `payment-sandbox` | ✅ |

| Assertion / execução | Mapeia para | Keep? |
| --- | --- | --- |
| `test_check_no_composite_build.py` (5 testes: real + 4 sintéticos de rejeição/aceitação) | MIG-05 (nenhum build source-level compartilhado) | ✅ |
| `test_legacy_transitional_roots_are_excluded` | ORG-08 (gate de equivalência não pode colidir/duplicar durante transição) | ✅ |
| `scripts/smoke.sh`, `scripts/e2e/async_redis_smoke.sh` (execução real, não simulada) | Done-when "≥2 E2E completos" | ✅ |

**Adequacy review:** cobertura suficiente e necessária. `check_no_composite_build.py` foi exercido contra as 4 fronteiras reais (não uma fixture sintética) e tem 4 testes de mutação sintética (`includeBuild` injetado, referência a fonte irmã injetada, arquivos ausentes, declaração limpa) que provam que o detector realmente rejeita o padrão proibido em vez de sempre retornar `[]`. Os dois E2E rodaram contra containers Docker reais construídos a partir de repositórios Maven publicados de verdade — não contra mocks nem contra o código-fonte compilado in-process — o que é exatamente o que MIG-05 exige provar. O fix do gate de equivalência foi verificado por hash (33 dos 44 "missing" resolvidos por SHA-256 idêntico no novo caminho) e por busca de basename com inspeção manual dos 11 restantes (movidos+modificados) antes de excluir qualquer coisa — nenhuma exclusão foi feita sem confirmar que o conteúdo existe em algum lugar rastreado. Os dois bugs de `compose.yaml` (JWT secret ausente, Postgres host errado) são achados legítimos desta tarefa, não escopo alheio: sem eles nenhum E2E rodaria, então corrigi-los é o próprio Done-when, não "já que estou aqui". SPEC_DEVIATION: nenhuma — o comportamento de todas as fronteiras permanece o mesmo sob os defaults documentados (`.env.example`); só preenchi placeholders que já eram exigidos pelo código e nunca tinham sido declarados.
...
**Gate evidence:** Construí `scripts/e2e/payment-failures/` como um harness de 11 cenários live, orquestrados por `run.sh`: `multi_instance.sh` sobe uma segunda instância real de `payment-api`/`payment-sbus` via `docker run` (não `--scale` — os Composes fixam porta de host), mesmas imagens `payment-api:local`/`payment-sbus:local` do T54, mesma rede `payment-sandbox`. `10/11` cenários passam — o único que falha é um bug real que encontrei e sinalizei como tarefa separada (`task_5a0df80c`, não corrigido aqui: corrigir código de produção é maior que o escopo desta tarefa, que é provar a matriz e documentar o que ela acha).

Achado real: com o Redis do sandbox indisponível, `RedisStatusStore.reserve()` (`payment-api/src/main/java/com/example/payments/api/redis/RedisStatusStore.java:98-124`) deixa a exceção do Lettuce propagar sem captura, resultando em `HTTP 500` com `"Unable to connect to redis/<unresolved>:6379"` vazado no corpo — não o `503`/`429` fail-closed que PAY-09 documenta (design.md §7.2, linha "Redis da API indisponível") e que o rate limiter já implementa (`AdmissionRedisOutageIT` pré-existente cobre só esse caminho, não o de reserva de idempotência, por isso o gap nunca foi pego antes). O cenário `redis-unavailable-api` afirma o comportamento *documentado* (fail-closed), não o atual — por isso ele falha de propósito, e a suíte inteira só passa porque `≥10` ainda sobra depois desse.

Cada cenário mapeia a um requisito e usa mecanismo real, não simulado:

- `duplicate-idempotency-key-cross-instance` / `cross-instance-fleet-coordination` (CAP-05): duas submissões verdadeiramente concorrentes (`&`/`wait`) com a mesma `Idempotency-Key` contra api-1 e api-2 resolvem para o mesmo `requestId`; 4 requisições round-robin entre as duas instâncias nunca colidem.
- `outbox-crash-window-reclaim` (PAY-05/06): reutiliza uma linha `outbox_event` real já `PUBLISHED` (payload Avro genuíno, não fabricado) e força seus campos de fencing de volta a um `IN_PROGRESS` obsoleto via `psql` direto — simula literalmente "morreu logo após o ack do Kafka, antes de marcar a outbox". O `OutboxReaper` real (lease 1m + reaper-interval 30s) reclama e republica a linha, e o resultado terminal já escolhido (`authorizationCode`) fica inalterado — prova as duas ACs com uma única manipulação, sem Testcontainers.
- `sbus-container-kill-mid-flight` (PAY-05): `docker kill` no sbus-1 em pleno voo; sbus-2 (`FOR UPDATE SKIP LOCKED` na mesma tabela) cobre a folga e a requisição chega a um estado terminal mesmo assim.
- `due-retry-does-not-block-live-traffic` (PAY-08): insere uma linha `outbox_event` com `next_attempt_at` no futuro (+2min) e confirma que uma requisição viva ao lado completa em segundos e a linha futura continua `PENDING`/não reclamada.
- `slow-core-backpressure` (CAP-06): recria `payment-core-mock` com latência 4-6s (acima do `wait-timeout: 3s` da API) via `docker compose up -d --force-recreate` com env override, dispara um burst de 15 requisições — todas voltam 200/202/422/429 (nunca 5xx) e todas completam depois, sem perda silenciosa.
- `poison-message-to-dlq` (PAY-07): publica bytes inválidos direto no tópico `payment.simulation.requested` via `kafka-console-producer.sh` (bypassando o path validado da API); a mensagem malformada gera uma linha `DLQ_PUBLISHED` nova, nunca desaparece.
- `kafka-unavailable-on-publish` / `redis-unavailable-api` / `postgres-unavailable-sbus` / `registry-unavailable` (PAY-09, uma dependência por vez, uma linha da matriz de falhas do design.md §7.2 cada): `docker stop`/`start` em cada dependência do sandbox isoladamente, submissão real pela API, restauração, e confirmação de recuperação. Kafka indisponível: `503` com corpo Problem+JSON depois de ~2min (`delivery.timeout.ms` default do produtor, não encurtado — é o comportamento real de produção, só o `PublishFailureIT` da IT encurta pra teste rápido) e o replay com a mesma `Idempotency-Key` completa depois que o Kafka volta. Postgres indisponível: a API aceita (Kafka ainda de pé) mas a requisição só assenta depois que o Postgres volta — sem falso ack, sem perda. Registry indisponível: `503` fail-closed; como `apicurio-registry-mem` é só em memória, um `stop`+`start` apaga todo schema registrado, então a recuperação depende de auto-registro no primeiro publish seguinte (documentado no script).

Duas rodadas completas da suíte com efeitos de ordem genuínos entre cenários (não flakiness aleatória): a primeira exposição achou que `RedisStatusStore.reserve()` sem timeout de reconexão vazava para o cenário seguinte (corrigido com um `sleep` de assentamento pós-recuperação do Redis) e que `sbus-container-kill-mid-flight` dispara um rebalance real do consumer group do Kafka que pode pausar o claim da frota inteira por dezenas de segundos (corrigido reordenando `due-retry-does-not-block-live-traffic` para antes dos cenários de crash/kill, já que sua janela de tempo não tem relação com esse rebalance). Também corrigi um bug de compatibilidade real: `bash 3.2` (padrão do macOS) trata `${arr[@]}` em array vazio como erro de variável não-setada sob `set -u` — removi `-u` de `run.sh`.

| Done-when / requisito | Evidência `file:line` e execução | Resultado definido | Coberto? |
| --- | --- | --- | --- |
| Done-when: toda aceitação termina ou fica recuperável | todos os 11 cenários usam `poll_terminal`/checagem de estado no DB, nunca assumem sucesso silencioso | cada requisição chega a `COMPLETED`\|`FAILED`\|`TIMEOUT`, ou o gap documentado é reportado como tal | ✅ |
| Done-when: ordem por requestId é preservada | `scenario_cross_instance_fleet_coordination` (`concurrency.sh:41-75`) — 4 requisições cross-instance, zero `requestId` duplicado | cada submissão independente recebe identidade própria, sem cross-talk entre fronts | ✅ |
| Done-when: nenhum cenário perde silenciosamente | `poison-message-to-dlq`, `kafka/postgres/registry-unavailable` todos verificam recuperação pós-restauração, não só a falha em si | toda falha injetada tem uma segunda asserção de recuperação | ✅ |
| Done-when: ≥10 cenários passam | execução real: `payment-failures: PASS (10/11 scenarios, floor is 10)` | 10 aprovados, 1 falha documentada e rastreada (`task_5a0df80c`) | ✅ |
| PAY-05/06: lease/ownership permite recuperação sem publicação concorrente | `crash_recovery.sh:6-44` execução real: `outbox-crash-window-reclaim: reclaimed+republished row ... without altering the already-chosen terminal result` | reclaim + republish não duplica nem corrompe o resultado terminal | ✅ |
| PAY-07: DLQ indisponível permanece recuperável, sem linha terminal silenciosa | `poison.sh` execução real: `malformed message routed to a new DLQ row, confirmed DLQ_PUBLISHED` | mensagem poison nunca desaparece, sempre chega a um estado confirmado | ✅ |
| PAY-08: retry due-based não bloqueia tráfego vivo | `crash_recovery.sh:86-110` execução real: `live request completed in 1s ... while the future-dated retry correctly stayed PENDING` | requisição viva não fica atrás do retry futuro na fila de claim | ✅ |
| PAY-09: timeout/retry/circuit compatíveis com a garantia documentada, uma dependência por vez | `dependency_outages.sh` execução real: Kafka `503` + replay recupera; Postgres aceita e assenta depois; Registry `503` + reregistro automático; Redis `500` (gap real, sinalizado) | 3 de 4 dependências já fecham a garantia; Redis tem gap real documentado e rastreado | ⚠️ 3/4, gap rastreado |
| CAP-05: ≥2 API + ≥2 SBUS, ordenação e coordenação cross-instance | `concurrency.sh` execução real contra api-1/api-2/sbus-1/sbus-2 reais (não Testcontainers) | idempotência e coordenação provadas com processos reais, não simulados | ✅ |
| CAP-06: Core abaixo da demanda converge para backlog limitado, sem SLO impossível | `core_backpressure.sh` execução real: 15/15 requisições sob Core 4-6s todas 200/202/422/429, todas completam, zero 5xx | backlog absorvido sem perda nem erro técnico | ✅ |

| Assertion / execução | Mapeia para | Keep? |
| --- | --- | --- |
| `scenario_duplicate_idempotency_key_cross_instance`, `scenario_cross_instance_fleet_coordination` | CAP-05 | ✅ |
| `scenario_outbox_crash_window_reclaim`, `scenario_sbus_kill_mid_flight` | PAY-05, PAY-06 | ✅ |
| `scenario_due_retry_does_not_block_live_traffic` | PAY-08 | ✅ |
| `scenario_slow_core_backpressure` | CAP-06 | ✅ |
| `scenario_poison_message_to_dlq` | PAY-07 | ✅ |
| `scenario_kafka_unavailable`, `scenario_redis_unavailable_api`, `scenario_postgres_unavailable_sbus`, `scenario_registry_unavailable` | PAY-09 (uma dependência por cenário) | ✅ |

**Adequacy review:** cobertura suficiente e necessária. Nenhum cenário usa mock ou Testcontainers isolado — todos batem em containers Docker reais, publicados a partir de repositórios Maven publicados (T54), na mesma rede que produção usaria. O cenário de crash-window reaproveita uma linha real (payload Avro genuíno) em vez de fabricar uma nova, evitando o risco de um payload sintético inválido poluir o fluxo; a prova foi feita antes de escrever o cenário (`psql` manual confirmando a estrutura de duas linhas por pagamento completo). O cenário de poison foi validado observando o comportamento real antes de fixar a asserção (a mensagem gerou uma linha DLQ nova que passou por `UNKNOWN_TOPIC_OR_PARTITION` antes de `DLQ_PUBLISHED`, confirmando que o teste não está apenas verificando um caminho feliz fabricado). O gap do Redis não foi escondido nem "consertado" às pressas dentro desta tarefa — corrigir `RedisStatusStore.reserve()` é uma mudança de código de produção que merece sua própria tarefa/spec, não um patch açodado para fazer a suíte "passar 11/11"; a asserção do cenário testa a garantia *documentada* (PAY-09/design.md), não o comportamento atual, e retornaria PASS automaticamente assim que `task_5a0df80c` for resolvida — sem precisar tocar neste arquivo de novo. SPEC_DEVIATION: nenhuma nos critérios cobertos; o gap de PAY-09/Redis é uma lacuna de implementação pré-existente descoberta agora, não um desvio desta tarefa.
...
**Gate evidence:** Construí `scripts/e2e/async-redis-failures/` como um harness de 10 cenários live (34 asserções), orquestrado por `run.sh`: `multi_instance.sh` sobe uma segunda instância real de `async-redis-service` via `docker run` (mesmo padrão do T55 — os Composes fixam porta de host, `--scale` não serve) para o cenário de identidade/reclaim cross-instance; os outros nove cenários sobem seus próprios containers scratch efêmeros com configuração específica (latência, pool, TTL, `max-deliveries`, `fail-on-reference`, `MICRONAUT_ENVIRONMENTS=prod`), porque os oito requisitos RED exigem ajustes mutuamente incompatíveis num único processo (ex.: TTL de segundos para provar expiração vs. TTL de produção para o resto). `10/10` cenários passam, `34/34` asserções — nenhum gap real encontrado nesta tarefa (diferente do T55): os oito RED já tinham sido implementados e testados via IT nas Fases 7 (T39-T44); esta tarefa prova os mesmos oito comportamentos contra containers Docker reais e coordenação cross-instance observada de fora (curl + `redis-cli`), não via beans injetados num único processo.

Achado de harness (não é bug de produto): a lease do reclaim é `reclaim:{group}:owner`, chaveada só pelo nome do *consumer group*, não pela stream. Um primeiro container scratch para o cenário de excesso de entregas (`ASYNC_MAX_DELIVERIES=1`) usava o grupo padrão `workers` e nunca teve sua vez de reclaim porque a frota principal (`async-redis-1`/`async-redis-2`, já rodando, também com grupo `workers`) renovava a lease continuamente. Corrigido dando a cada container scratch que depende de reclaim seu próprio nome de grupo (`ASYNC_REDIS_GROUP`). Isso nunca acontece em produção — uma frota real deliberadamente compartilha um grupo para uma stream — só afeta um harness que sobe implantações logicamente separadas lado a lado; documentado no topo de `scenarios/dlq.sh`.

Achado de robustez do próprio harness: o cenário RED-02-bônus (`admission-limit-per-sec` compartilhado entre instâncias) inicialmente falhou quando rodado como último estágio de `verify-workspace.sh all` (depois de ~10-15min de atividade Docker pesada dos estágios anteriores) — sob essa carga, a rajada paralela de 10 requisições podia atravessar a fronteira de segundo da janela fixa (1000ms, chaveada por segundo Unix) do rate limiter, inflando a contagem aceita além do limite configurado sem que isso fosse um gap real do produto. Corrigido alinhando a rajada ao início de um novo segundo (`_t56_rl_wait_for_second_boundary`) com até 3 tentativas se uma rajada ainda assim atravessar a fronteira — `scripts/e2e/async-redis-failures/scenarios/rate_limit.sh:11-23,60-77`. Reproduzi a falha original, apliquei o fix, e rodei a suíte completa (`verify-workspace.sh all`, todos os 8 estágios) duas vezes limpas depois — `34/34` nas duas.

Cada cenário mapeia a um ou mais requisitos e usa mecanismo real, não simulado:

- `status-lifecycle-all-states` (RED-01): dois containers scratch — um com latência inflada (5-6s, acima do `wait-timeout` de 3s) prova que o POST retorna `202 PROCESSING` dentro do orçamento HTTP e que um `GET` imediato confirma o mesmo status antes do término; outro com `result-ttl=3s`/`status-ttl=8s` prova `404 UNKNOWN` (nunca submetido), `200 COMPLETED` e `410 EXPIRED` (depois do `result-ttl`, antes do `status-ttl`) — as quatro saídas do RED-01 observadas ao vivo.
- `wait-pool-backpressure` (RED-02): pool com `pool-max-total=1`/`pool-max-wait=300ms` contra uma latência de 4-4.5s; uma segunda requisição concorrente retorna em ~0.3s (medido) com `X-Backpressure: wait-pool-exhausted`, e o job aceito sob backpressure completa de forma assíncrona mesmo assim.
- `retention-alert-without-autotrim` (RED-03): `stream-maxlen=5` real, 10 jobs entram na stream — `XLEN` nunca cai abaixo de 10 (nenhum auto-trim, mesmo passando o `maxlen`) e o monitor agendado grava o alerta de backlog no log real do container.
- `cross-instance-worker-identity` (RED-04): com `async-redis-2` de pé, `XINFO CONSUMERS` no Redis real mostra 4 consumers distintos no padrão `<instance-id>-w<index>`, e `reclaim:workers:owner` sempre contém exatamente um dono, inclusive depois de uma janela de renovação de lease.
- `redis-outage-worker-readiness` (RED-05): `docker stop`/`start` no Redis real do sandbox; `/health/readiness` cai para `503` (`async-redis-workers: DOWN`, `consumingWorkers: 0`) e recupera para `200` depois do reconnect com backoff — confirmado que um job submetido pós-recuperação completa de ponta a ponta.
- `atomic-release-survives-ownership-theft` (RED-06): rouba a posse do PEL de uma mensagem real em processamento via `XCLAIM` para um consumer forjado (`rogue-consumer`) enquanto ainda está em voo; o worker original ainda assim libera o resultado exatamente uma vez (`resp:{id}:sent` = 1, `LLEN resp:{id}` = 1) e o `XACK` limpa a mensagem do PEL independente de quem "possuía" a entrada — prova que `XACK` não exige posse do PEL, só a idempotência real do `ResultReleaser` protege contra duplicação.
- `malformed-message-dlq-before-ack` / `exceeded-deliveries-dlq` (RED-07): `XADD` direto na stream (bypassando a API) com `jobId` ausente gera DLQ com `dlqReason: missing-job-id` na primeira entrega, sem deixar nada pendente; um job com `fail-on-reference` ativo e `max-deliveries=1` é DLQ'd com `dlqReason: max-deliveries-exceeded` depois de esgotar as tentativas, também sem deixar resto no PEL.
- `admission-limit-shared-across-instances` (RED-02, bônus): duas instâncias reais compartilhando `admission-limit-per-sec=5` — uma rajada de 5+5 alinhada à janela do segundo aceita ~5 no total (não ~10), provando que o limite é atômico via Redis, não um contador local por instância, conforme o próprio comentário de `AsyncRateLimiter.java`.
- `production-guard-rejects-insecure-config` (RED-08): quatro containers reais em `MICRONAUT_ENVIRONMENTS=prod` — chave dev-default, `idempotency-required=false` e `admission-limit=0` cada um individualmente recusa o startup (`docker inspect` mostra `exited`); um quarto com os três gates corretos sobe `healthy` (controle positivo).

| Done-when / requisito | Evidência `file:line` e execução | Resultado definido | Coberto? |
| --- | --- | --- | --- |
| Done-when: todos os oito ACs RED têm evidência observável | 10 cenários acima, cada um citando comando/log/resposta HTTP real | RED-01..08 todos observados contra containers/Redis reais, não beans injetados | ✅ |
| Done-when: ≥10 cenários passam | execução real: `async-redis-failures: PASS (34/34 assertions, floor is 10)` | 10 cenários, 34 asserções, todas passam | ✅ |
| Done-when: sem PEL/pool ilimitado | `atomic-release-survives-ownership-theft` e `dlq.sh` confirmam PEL vazio (`XPENDING` vazio) depois de cada cenário; `wait-pool-backpressure` confirma pool fixo (`pool-max-total=1`) nunca bloqueia além do orçamento | nenhum cenário deixa PEL crescendo ou pool sem teto | ✅ |
| RED-04: coordenador único de reclaim entre instâncias | `scenarios/identity.sh` execução real: `reclaim:workers:owner` sempre um único valor com 2 instâncias + 4 workers de pé | apenas um dono por vez, mesmo sob renovação de lease | ✅ |
| RED-05: readiness cai/recupera numa queda real do Redis compartilhado | `scenarios/redis_outage.sh` execução real: `503` -> `200` via `docker stop`/`start` no Redis do sandbox | DOWN durante a queda, UP após reconexão, job pós-recuperação completa | ✅ |

| Assertion / execução | Mapeia para | Keep? |
| --- | --- | --- |
| `scenario_status_lifecycle_all_states` (6 asserções) | RED-01 | ✅ |
| `scenario_wait_pool_backpressure` (3 asserções) | RED-02 | ✅ |
| `scenario_retention_alert_without_autotrim` (2 asserções) | RED-03 | ✅ |
| `scenario_cross_instance_worker_identity` (4 asserções) | RED-04 | ✅ |
| `scenario_redis_outage_worker_readiness` (4 asserções) | RED-05 | ✅ |
| `scenario_atomic_release_survives_ownership_theft` (5 asserções) | RED-06 | ✅ |
| `scenario_malformed_message_dlq_before_ack`, `scenario_exceeded_deliveries_dlq` (2+2 asserções) | RED-07 | ✅ |
| `scenario_admission_limit_shared_across_instances` (2 asserções) | RED-02 (bônus) | ✅ |
| `scenario_production_guard_rejects_insecure_config` (4 asserções) | RED-08 | ✅ |

**Adequacy review:** cobertura suficiente e necessária. Nenhum cenário reimplementa o que as ITs da Fase 7 já provam em processo único — cada um prova o mesmo comportamento contra containers Docker reais, Redis real e coordenação observada de fora (curl + `redis-cli`), que é exatamente o que estava faltando (a tabela de mapeamento ITs-existentes-vs-harness-live foi levantada antes de escrever qualquer cenário). O cenário de RED-06 foi o mais delicado de validar: a primeira tentativa usou uma `Idempotency-Key` fixa reaproveitada entre execuções manuais de debug, o que causou replay do job antigo em vez de um novo enqueue — descoberto e corrigido (todas as chaves de idempotência do harness agora incluem `$RANDOM`) antes de aceitar qualquer resultado como evidência; o roubo de posse via `XCLAIM` foi confirmado por inspeção direta do PEL antes e depois (`XPENDING` mostrando o novo dono), não assumido. O achado de escopo do reclaim lease (por grupo, não por stream) foi verificado isolando a causa (rodei o cenário de excesso de entregas sozinho vs. com a frota principal de pé) antes de aplicar o fix, e o achado de robustez do rate-limit foi reproduzido (falhou de forma determinística como último estágio da suíte completa) antes e depois do fix (duas execuções limpas de `verify-workspace.sh all`). SPEC_DEVIATION: nenhuma — os oito requisitos RED já estavam implementados corretamente (Fases 7); esta tarefa é prova cross-boundary, não implementação. Os dois achados acima são do próprio harness de teste, não do produto, e ficam documentados inline nos scripts para o próximo mantenedor.
...
**Gate evidence:** Construí `load/capacity/` (manifest, `lib.sh`, `collect_metrics.sh`, `reconcile.py`, `generate_report.py`, `run_gate.sh`, `scenarios/{steady,spike,soak,slowdown,recovery}.sh`) e `load/k6/capacity.js`, orquestrando dois perfis reais (`certified-target`, `constrained-core`) contra uma frota real de 2x `payment-api` + 2x `payment-sbus` + `payment-core-mock` (mesmo padrão `docker run` do T55/T56), com steady 167 req/s×15min e spike 333 req/s×60s automatizados no valor literal de AD-006, mais soak/slowdown/recovery com duração dimensionada por engenharia (documentado em `manifest.yaml#scenarios`). `generate_report.py aggregate` calcula os thresholds definidos pelo spec (erro técnico <0,1%, zero perda silenciosa amostrada — CAP-02) e falha o gate (`exit 1`) quando violados; `generate_report.py selftest` prova deterministicamente que esse caminho de falha funciona (CAP-07), sem depender de uma execução ao vivo:

```
$ python3 load/capacity/generate_report.py selftest
{"good_passed": true, "good_reasons": [], "bad_passed": false,
 "bad_reasons": ["technical error rate 0.5000% >= 0.1000% (CAP-02)",
                 "10/2000 sampled requests never reached a terminal state (CAP-02 zero silent loss)"],
 "selftest": "PASS"}
```

Três bugs reais de harness foram encontrados e corrigidos antes da execução valer como evidência (auditoria completa, sem atalho): (1) `reconcile.py` fazia polling sequencial de cada `requestId` amostrado — contra o backlog de ~105.000 itens que o próprio `constrained-core` gera de propósito (design.md §6.1), isso travava por horas; corrigido com `ThreadPoolExecutor` (`reconcile.py:75-83`), limitando o tempo total a `max_polls` segundos em vez de `amostras × max_polls`. (2) o parser do resumo do k6 assumia um schema `{"values":{"count":...}}` de uma versão antiga do k6; o k6 0.54 real emite `{"count":...}` direto — `metric_count`/`metric_trend` corrigidos (`generate_report.py:37-51`) depois de comparar contra um `k6 run` real, não contra suposição. (3) `collect_metrics.sh` emitia JSON multi-linha; `recovery.sh` acumula um snapshot por linha (JSONL) — corrigido compactando com `jq -c` (`collect_metrics.sh:48-49`).

Execução real completa (`load/reports/run_gate.txt`, `load/reports/20260811-185714-capacity-report.md`, 4389s, ambos os perfis, todos os cinco cenários): **FAIL** — taxa de erro técnico entre 4% e 44% em `certified-target` (steady/spike/soak/slowdown), muito acima do limiar de 0,1% do CAP-02. Investigado antes de aceitar como resultado final: `p95`/`p99` grudados em ~60000-60011ms (timeout HTTP padrão do k6) e o log do k6 registrando `"Insufficient VUs, reached 760 active VUs and cannot initialize more"` descartaram falta de VUs simples; dobrar CPU de 1.0 para 2.0 por container não mudou o padrão (reproduzido de forma idêntica numa checagem dirigida de 90s/167rps — `load/reports/certified-target/cpucheck.*`), descartando orçamento de CPU. A causa real, confirmada no log do próprio `payment-api-2`: `ResponseCoordinator.complete()` (`payment-api/src/main/java/com/example/payments/api/coordination/ResponseCoordinator.java:119`) chama `RedisStatusStore.get()` (`payment-api/src/main/java/com/example/payments/api/redis/RedisStatusStore.java:73`) de forma síncrona dentro do callback do listener PubSub do Lettuce; `RedisStatusStore` mantém uma única conexão sem pool e sem timeout customizado, que sob carga concorrente sustentada acumula fila até bater no timeout padrão de 60s do Lettuce (`RedisCommandTimeoutException`), travando a entrega de notificações de conclusão daquela instância — evidência completa em `load/reports/certified-target/cpucheck.redis-timeout-evidence.txt`, recorrendo a cada ~60s (20:18:11, 20:19:11, 20:20:11, 20:21:12) sem se recuperar sozinho.

| Done-when / requisito | Evidência `file:line` e execução | Resultado definido | Coberto? |
| --- | --- | --- | --- |
| Done-when: steady 10k/min×15m e spike 20k/min/60s automatizados | `load/capacity/scenarios/steady.sh` (rate=167, 15m), `spike.sh` (rate=333, 60s); execução real `run_gate.txt` | valores literais de AD-006, não amostra reduzida | ✅ |
| Done-when: thresholds falham corretamente | `generate_report.py selftest` (determinístico) + execução real: `Verdict: FAIL` com 5 violações citadas | caminho de falha provado duas vezes, por vias independentes | ✅ |
| Done-when: relatório com hardware/recursos/percentis/lag/backlog/GC/pools/DB | `20260811-185714-capacity-report.md` — cada cenário lista throughput, percentis p50/p95/p99, mix de status, `api_pending`/`sbus_outbox_pending`/`sbus_dlq_unconfirmed`, heap/GC, Hikari, Redis, Postgres | todos os sinais do CAP-04 presentes por cenário | ✅ |
| CAP-05: ≥2 API + 2 SBUS, ordenação por requestId cross-instance | `steady.sh` sonda de chave duplicada concorrente contra as duas instâncias: `PASS` nos dois perfis (`requestId` idêntico) | mesma identidade resolvida por qualquer instância, sob carga | ✅ |
| CAP-06: admissão converge para backlog limitado sob Core insuficiente | `constrained-core` (Core capado a 50/s) roda todos os cenários sem crash; `slowdown` prova drenagem <1s após restaurar Core (backlog pequeno por desenho, ver nota abaixo) | comportamento observado, sem prometer SLO terminal impossível | ✅ |
| CAP-02: erro técnico <0,1%, zero perda silenciosa | Violado em `certified-target` — causa raiz diagnosticada e citada acima | gap real de produto encontrado, não meta atingida | ❌ (gap real, ver SPEC_DEVIATION) |

**Adequacy review:** a tarefa pedia construir e provar o gate, não certificar que a plataforma passa — "thresholds falham corretamente" é literalmente um dos critérios de "Done when", e é isso que a execução real demonstrou. Descartei as duas explicações alternativas mais prováveis (VUs insuficientes, CPU insuficiente) com evidência antes de aceitar a causa Redis: a primeira foi refutada porque o padrão de erro (100% das falhas numa única instância, `payment-api-2`, nunca `payment-api-1`) não é compatível com um limite simétrico de concorrência; a segunda foi refutada reproduzindo o mesmo padrão depois de dobrar o CPU. Só então fui ao log do container, onde o stack trace aponta exatamente para a linha responsável. O cenário `slowdown` foi redimensionado (de 100 req/s para 2 req/s, de 4min para 90s) depois que a primeira tentativa real revelou que, com uma única partição Kafka e consumo síncrono bloqueante (`CoreSimulationConsumer.java:63-71`), a latência injetada de 1,5-3,5s *é* o teto de throughput do Core (~0,3-0,7 msg/s) — a config original geraria backlog de ~24.000 itens, não drenável na janela de `recovery`; documentado em `slowdown.sh:1-17`. **SPEC_DEVIATION:** o achado de CAP-02 (`RedisStatusStore`/`ResponseCoordinator`, conexão Redis sem pool nem timeout) é um gap real de `payment-api`, fora do escopo de `load/` (a mesma régua que T55 aplicou ao seu próprio achado em `RedisStatusStore`) — não corrigido nesta tarefa, rastreado como **task_3801253b**.

**Update (mesma sessão, fora do escopo formal de T57):** `task_3801253b` foi corrigido — `ResponseCoordinator.onMessage()` agora despacha para uma virtual thread em vez de rodar inline na thread de event-loop do PubSub do Lettuce, e `RedisClientTuning` limita todo comando Lettuce a 2s em vez do padrão de 60s. Reproduzi a cena original (167 req/s×90s contra `certified-target`) ao vivo depois da correção: `docker logs payment-api-1 | grep -c RedisCommandTimeoutException` = 0 (antes recorria a cada ~60s), `cap_technical_errors` = 0 (antes 40-44%). Suíte completa de `payment-api` (`./gradlew test -PwithIT`) permanece verde, 118/118, incluindo dois testes novos de regressão (`ResponseCoordinatorUnitTest`) que provam que uma notificação lenta para um `requestId` nunca bloqueia a de outro, e um teste estrutural (`RedisClientTuningUnitTest`) confirmando o timeout limitado.
...
**Gate evidence:** Construí `scripts/observability/` (lib.sh, verify.sh, seis `checks/*.sh`) contra uma frota real de uma instância de cada app (`docker compose up` em `payment-core-mock`/`payment-sbus`/`payment-api`, mesma rede sandbox), e fechei dois gaps de infraestrutura que STATE.md já apontava como pendentes de T58: `sandbox/observability/application-targets.json` (estava `[]`) agora declara scrape targets reais para os quatro apps, e `sandbox/observability/application-assets.json` (também `[]`) agora declara owner/version/path dos alertas de cada app, montados (não copiados — `sandbox/docs/observability.md`) no Prometheus do sandbox via `sandbox/compose.profiles.yml` + `rule_files` em `sandbox/observability/prometheus.yml`. Execução real (`scripts/observability/verify-evidence.txt`): **10/12 checks passam** (chão de 8 atingido), as 2 falhas são gaps reais e documentados, não bugs de harness (detalhe abaixo).

Três bugs reais de harness corrigidos durante a construção (auditoria completa, sem atalho): (1) o scrape do Prometheus (tanto o do sandbox quanto o Prometheus efêmero do teste de alerta) usava o path padrão `/metrics`, mas todo app deste workspace expõe métricas em `/prometheus` (convenção Micronaut) — corrigido com `metrics_path: /prometheus` em ambos (`sandbox/observability/prometheus.yml`, `checks/alert_fires.sh`). (2) o teste de disparo de alerta falhava mesmo com o mecanismo correto, porque `api_response_dead_lettered_total` é um contador Micrometer com tag dinâmica (`ApiMetrics.java:76-78`) que só passa a existir no Prometheus na primeira ocorrência — `increase()` precisa de duas amostras da mesma série pra calcular crescimento, e uma série que acabou de nascer com uma amostra não basta; corrigido envenenando duas vezes (a primeira cria a série, a segunda prova o crescimento observado) em vez de uma. (3) `scripts/docs/validate_docs.py#executable_corpus` (T5) usava um glob de profundidade fixa (`*/src/main/java/**`) que nunca alcançava `feature-control/library/src/main/java/...` (dois níveis) nem `feature-control/examples/*/src/main/java/...` (três níveis) — isso já causava uma falha silenciosa e pré-existente do próprio `test_validate_docs.py` (confirmada rodando a suíte no commit anterior, sem minha mudança: `feature_decisions_total` não verificado); corrigido para `**/src/main/java/**` (confirmei que nenhum diretório `build/` gerado colide com esse padrão mais amplo antes de aplicar).

Dois achados reais de produto, não corrigidos nesta tarefa (fora do escopo de `scripts/observability`, mesma régua que T55/T57 aplicaram aos próprios achados): `payment-sbus`'s `Mdc.java` (utilitário completo para popular requestId/correlationId/causationId/traceId no MDC) nunca é chamado em lugar nenhum do código — `grep -rn "Mdc\." payment-sbus/src/` não retorna nada; toda linha de log estruturado do SBUS carece desses campos como chaves JSON. E `payment-api` nunca popula `causationId` no MDC (declarado em `logback.xml`, nunca setado em `ApiPaymentService.java:113-115` nem `PaymentResponseConsumer.java:146-150`), embora o dado exista corretamente no `EventEnvelope`. Ambos rastreados juntos como **task_89c681c8** (mesma classe de problema: logging de correlação estruturado incompleto).

**Update (mesma sessão, fora do escopo formal de T58):** `task_89c681c8` foi corrigido — `SimulationMessageHandler.handleRequested`/`handleCoreResponse` agora chamam `Mdc.fromConsumer(record, env)` antes de processar e `Mdc.clear()` no `finally` (mudança de assinatura em `handle()` para receber o `ConsumerRecord` inteiro, simplificando os três listeners que só repassavam `value`+`headers` extraídos manualmente); `ApiPaymentService.publishAndAwait` e `PaymentResponseConsumer.apply` agora colocam `causationId` no MDC. Testes novos: `MdcUnitTest` (payment-sbus, prova o utilitário), `ApiPaymentServiceUnitTest.populatesCausationIdInMdcWhilePublishing` (captura o MDC no momento do publish real). Gates completos (`./gradlew test -PwithIT`) de ambas as fronteiras verdes. Reproduzi ao vivo depois de reconstruir as duas imagens: `scripts/observability/verify.sh` foi de 10/12 para **12/12**.

| Done-when / requisito | Evidência `file:line` e execução | Resultado definido | Coberto? |
| --- | --- | --- | --- |
| Done-when: request/correlation/trace propagam | `checks/correlation_ids.sh` execução real: payment-api log carrega requestId+correlationId+traceId | 3 dos 4 campos confirmados via log JSON real | ✅ |
| Done-when: causation propaga | `checks/correlation_ids.sh` — payment-api nunca popula causationId | gap real encontrado e citado, não escondido | ❌ (gap real, task_89c681c8) |
| Done-when: alertas críticos disparam sinteticamente | `checks/alert_fires.sh` execução real: `PaymentApiResponseDeadLettered` (`for: 0m`) atinge `state=firing` contra regra real, métrica real, gatilho real (poison de `payment.simulation.completed`) | disparo provado ao vivo, não simulado | ✅ |
| Done-when: dashboard quebrado falha | `checks/dashboard_validator.sh` — fixture real injetada (`this_metric_was_removed_and_must_be_detected_t58`) detectada, depois removida sem deixar rastro; dashboards reais confirmados limpos (após o fix do glob) | EDG-04 provado de ponta a ponta com fixture real, não só string sintética | ✅ |
| Done-when: ≥8 checks passam | `verify-evidence.txt`: `RESULT: 10 passed, 2 failed` | 10/12, chão atingido | ✅ |
| SBX-05: sandbox carrega apenas ativos de app por mecanismo documentado/versionável | `application-targets.json`/`application-assets.json` populados; `checks/assets_manifest.sh` confirma owner+version+kind+path resolvendo para arquivo real | mecanismo documentado em `sandbox/docs/observability.md` agora ativado, não só descrito | ✅ |

**Adequacy review:** cobertura suficiente e necessária — cada check mapeia a um pedaço específico do Done-when, sem checks especulativos. A causa dos dois achados de correlação foi confirmada por `grep` direto no código-fonte antes de aceitar como gap real (não assumida por inspeção do log sozinho); o achado do glob do T5 foi confirmado rodando `test_validate_docs.py` no commit anterior à minha mudança para provar que já falhava sem mim. O teste de disparo de alerta foi depurado isolando cada variável (nome de container, path de métricas, semântica de `increase()` sobre série nova) com evidência de `/api/v1/targets`/`/api/v1/query` em vez de tentativa-e-erro às cegas. SPEC_DEVIATION: nenhuma nos critérios cobertos por esta tarefa — os dois gaps de correlação são lacunas de implementação pré-existentes descobertas agora, e o gap de auth para scrape de `/prometheus` (documentado em `application-assets.json`, ver `scrape_auth_status`) é uma limitação conhecida do modelo de segurança atual, não algo que `scripts/observability` deva ou possa corrigir.
...
**Gate evidence:** Escopo real acabou maior que a descrição sugeria: 244 seções de doc pendentes (não as ~7 esperadas), gate de equivalência vermelho por dois globs desatualizados, CI ainda hardcoded para caminhos legados e 10 dashboards não realocados. Com sub-agentes paralelos (um por fronteira dona), migrei as 251 seções rastreadas do manifest de relocation (`scripts/docs/relocation-manifest.json`, todas `MIGRATED`), realoquei os 10 dashboards Grafana via `git mv` (preservando histórico; dois renomeados — `redis-overview.json` -> `payment-api/ops/dashboards/api-waiters.json`, `postgres-overview.json` -> `payment-sbus/ops/dashboards/postgres-pool.json` — porque os nomes antigos não refletiam as métricas reais), reescrevi `.github/workflows/ci.yml`/`dependabot.yml` para as 7 raízes standalone, reescrevi `README.md`/`AGENTS.md` raiz sem framing "transicional", e removi fisicamente `common/`, `api-service/`, `sbus-service/`, `core-mock/`, `observability/` e os arquivos de build raiz (`git rm`, preservando histórico). Corrigi 4 bugs de path hardcoded expostos pela própria remoção: `equivalence.py`'s glob de `dashboards` (apontava para o `observability/grafana/dashboards/` legado), `build_relocation_manifest.py` (crashava em source ausente), `validate_docs.py#manifest_errors` (reportaria 251 falsos "stale" sem um branch de estado terminal) e `known_ports()`/`known_variables()` (liam o `docker-compose.yml` raiz deletado). `scripts/artifacts/verify-artifact-only.sh` também dependia do `gradlew` raiz deletado (dois usos `-p <dir>` fora da árvore); reescrito para usar o wrapper do próprio `feature-control` (correto: `publishMavenPublicationToLocalBuildRepository` continua `:feature-control:...` porque `feature-control/settings.gradle` mantém o path do projeto da lib como `:feature-control`), emprestado como launcher genérico para o fixture `consumer-fixture` também. `equivalence: PASS (409 entries)`, `docs: PASS (251 sections)`, `check_root_governance.py` PASS, `check_ci_policy.py` PASS. Execução completa e ao vivo do gate final (`scripts/verify-workspace.sh`, todas as 8 stages, com os 4 apps reconstruídos e subidos via `docker compose --env-file .env up --build` contra artefatos recém-publicados): equivalence PASS, no-composite-build PASS (4 boundaries), artifact-only-consumer PASS, e2e-payment `SMOKE OK (COMPLETED)`, e2e-async-redis `SMOKE OK (COMPLETED)`, payment-failures PASS (10/11, chão é 10 — a única falha é `redis-unavailable-api`, gap pré-existente já rastreado como follow-up, não uma regressão desta tarefa), async-redis-failures PASS (34/34 assertions), hygiene (`git diff --check`) PASS.
...
**Gate evidence:** Rodei os 6 gates Quick+Full de fronteira Java ao vivo contra o layout final pós-T59 (`payment-contracts` 28/28+28/28, `payment-api` 68/68+119/119, `payment-sbus` 44/44+76/76, `payment-core-mock` 21/21+27/27, `feature-control` 113/113+141/141, `async-redis-service` 39/39+96/96 — todos verdes). As primeiras tentativas ao vivo produziram falhas falsas em 3 das 6 fronteiras, todas rastreadas a estado desta própria sessão (não a regressão de código): colisão de porta com containers `:local` deixados rodando pela verificação do T59 (`payment-api` porta 8080, `async-redis-service` porta 8084, `pilot-app` porta 8085 colidindo com o registry do sandbox), variáveis de ambiente exigidas sem default que `docker compose`/CI fornecem mas uma chamada direta a `./gradlew` não (`PAYMENT_API_KEY`, `JWT_SIGNATURE_SECRET`), e um timeout de asserção sob load average 20+ (12 núcleos) causado por ~11 containers Testcontainers órfãos rodando havia 10 horas de trabalho anterior nesta sessão. Corrigi cada causa (portas liberadas, env vars exportadas, containers órfãos removidos) e re-executei limpo. Reproduzi localmente o mecanismo SBOM+scan que o CI de cada fronteira já usa (`syft` + `trivy image --severity HIGH,CRITICAL --ignore-unfixed`, mesma política do `exit-code: 1` do `trivy-action`) contra as 4 imagens `:local`: **achado real, registrado como FAIL, não promovido a PASS** — todas as 4 imagens têm entre 25 e 31 vulnerabilidades HIGH/CRITICAL com correção já publicada (Netty, Micronaut, Jackson, Kafka clients, Avro, driver Postgres); o próprio gate do CI bloquearia essas imagens hoje. Rastreado como `task_40100c4c` (bump de dependências + regressão completa, fora do escopo de coleta de evidência do T60). Reaproveitei o `scripts/verify-workspace.sh` do T59 (commit `add6749`, mesmo layout, nada mudou na árvore de dependências desde então) para equivalence/no-composite-build/artifact-only-consumer/e2e/failure-matrices/hygiene, o `load/reports/20260811-185714-capacity-report.md` do T57 para capacidade, e `validate_docs.py`/`check_root_governance.py`/`check_ci_policy.py`/`actionlint` do T59 para docs/governança/CI — nenhum desses mudou de resultado. Relatório completo: `validation-evidence/2026-08-12-release-gate.md`. Todos os 77 requisitos permanecem em `Execute` na tabela de rastreabilidade de `spec.md` (nenhum pendente de design ou não mapeado).

**Verifier fix cycle (mandatory, author ≠ verifier — `.specs/features/repository-segregation-production-hardening/validation.md`):** o Verifier independente rodou após T60 (último task da feature) e retornou **FAIL** com achados reais: 2 ACs não provados (CAP-02, EDG-04), 1 mutante sobrevivente (wiring de MDC do SBUS sem teste de call-site) e evidências obsoletas (ORG-08, `verify-evidence.txt`). Corrigi os 7 gaps ranqueados:
1. **EDG-04/Gap 2 (blocker)** — `validate_docs.py#dashboard_metric_errors` ainda apontava para `observability/grafana/dashboards/` (deletado pelo T59), escaneando 0 de 10 dashboards reais e retornando PASS vazio; corrigido para os 3 globs reais (mesmo padrão do `equivalence.py`), e `dashboard_validator.sh`'s `DASHBOARDS_DIR` realocado para `sandbox/observability/dashboards`; re-verificado ao vivo (`dashboard_validator.sh` PASS, fixture detectada de novo).
2. **CAP-02/Gap 1 (blocker)** — a claim de "0 técnicos" pós-fix nunca virou artefato commitado. Re-rodei o cenário `certified-target/steady` completo (167 req/s×15min, 150.301 requisições) ao vivo: **technical error rate = 0.0000%** (`load/reports/20260812-095631-capacity-report.md`), sub-critério provado. O veredito automático do próprio `generate_report.py` ainda é FAIL por "zero perda silenciosa" (2/185 amostras); investiguei via Postgres direto e confirmei que ambas completaram em ~1s — falso negativo da própria metodologia de reconciliação (status-ttl do payment-api = 15m, igual à duração do cenário, então amostras do início do run expiram do Redis antes da reconciliação checar no fim). Não é regressão nova; documentado em `load/reports/20260812-095631-cap02-addendum.md`, correção da metodologia rastreada como `task_bca58451`.
3. **Gap 5 (major, mutante sobrevivente)** — `MdcUnitTest` só provava o contrato do utilitário, nada provava que `SimulationMessageHandler` de fato chamava `Mdc.fromConsumer()`. Adicionei `SimulationMessageHandlerUnitTest.handlingARequestedEventPopulatesMdcWithTheEnvelopeCorrelationIdsWhileProcessing` (captura o MDC via `doAnswer` no mock do service, no exato padrão que `ApiPaymentServiceUnitTest` já usava do lado da API). Verifiquei empiricamente que mata a mutação: reproduzi a remoção dos dois call-sites em um `git worktree` isolado — o novo teste falhou como esperado; árvore real nunca tocada (`git status --porcelain` idêntico antes/depois).
4. **Gap 3 (major)** — `scripts/observability/verify-evidence.txt` ainda registrava o run pré-fix (10/12, duas falhas já corrigidas por `b6d5836`). Re-rodei `scripts/observability/verify.sh` ao vivo: **12/12**, regenerado.
5. **Gap 4 (minor)** — `ORG-08` citava "389 entries"; corrigido para 409 (valor real desde a reconciliação T57/T58).
6. **Gap 6 (minor)** — `LEGACY_TRANSITIONAL_ROOTS` e seu teste em `equivalence.py`/`test_equivalence.py` ficaram mortos após o T59 deletar as raízes legadas de vez; removidos.
7. **Gap 7 (minor, pré-existente)** — `ci.yml`'s matriz de integração do `feature-control` excluía os 20 ITs da própria library; corrigido para `./gradlew test -PwithIT` (mesma mudança espelhada em `check_ci_policy.py`'s `required_integration`).

Todos os fixes verificados ao vivo (gates re-executados, não apenas lidos). Baseline de equivalência regenerada duas vezes (uma por batch de arquivo tocado) — `equivalence: PASS (409 entries)` em ambas. Re-dispatch do Verifier é o próximo passo antes de considerar a feature `done`.
