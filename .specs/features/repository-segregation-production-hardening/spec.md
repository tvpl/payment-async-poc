...
| CAP-02 | Capacidade | Execute | T57 ran steady (167 req/s×15min) live against `certified-target`: **FAIL** — technical error rate 4-44%, root-caused to a real `payment-api` gap (`ResponseCoordinator.complete()` / `RedisStatusStore.get()`, unpooled Redis connection hits Lettuce's default 60s command timeout under sustained load — `payment-api/src/main/java/com/example/payments/api/coordination/ResponseCoordinator.java:119`, `payment-api/src/main/java/com/example/payments/api/redis/RedisStatusStore.java:73`); evidence in `load/reports/20260811-185714-capacity-report.md`; not fixed in T57 (out of `load/`'s scope), tracked as `task_3801253b` |
