...
| SEC-08 | Segurança e operação | Execute | T30 SBUS, T37 API and T45 async Redis CI build the image, generate an SPDX SBOM and block HIGH/CRITICAL Trivy findings; T60 reproduced this tooling live against all 4 app images (`syft`+`trivy image --severity HIGH,CRITICAL --ignore-unfixed --exit-code 1`, matching CI's config) — the gate itself works correctly and is wired into every boundary's CI, but the current dependency versions fail it: 25-31 HIGH/CRITICAL findings per image, all with a fix already published upstream (Netty, Micronaut, Jackson, Kafka clients, Avro, Postgres driver). Recorded as a real FAIL, not silently promoted to PASS; tracked as a follow-up (`task_40100c4c`) since a coordinated dependency bump + full regression across 4 boundaries is out of T60's own evidence-gathering scope. Full detail: `validation-evidence/2026-08-12-release-gate.md` §3 |
