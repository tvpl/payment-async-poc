 ...
 * </ul>
 *
 * <p>On {@code PERCENTAGE}/{@code VARIANT} flags, a non-empty allowlist acts as an <em>override</em>
 * that pins matching users/groups to the "on" side regardless of the percentage — the standard
 * "internal users always get the new thing" behavior. An unknown or disabled flag resolves "off",
 * so a missing flag can never accidentally enable a code path.
 */
@Singleton
public class FeatureResolver {
    ...
    public FeatureResolver(FlagSource flagSource) {
    ...
    private FeatureDecision percentage(String flag, FlagDefinition def, FeatureContext ctx) {
        ...
        int bucket = Bucketer.bucket(flag, ctx.bucketingKey());
    ...
    private FeatureDecision variant(String flag, FlagDefinition def, FeatureContext ctx) {
        ...
        Variant chosen = Bucketer.select(def.variants(), flag, ctx.bucketingKey());
        if (chosen == null) {
