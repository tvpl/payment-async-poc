...
if [ "${1:-}" = "--structural" ]; then
  echo "sbus-release-package: PASS (structural)"
  exit 0
fi

project_name="payment-sbus-t30"
cleanup() {
...
docker compose --project-name "$project_name" --env-file .env.example \
  -f compose.yaml up --build --detach --wait --wait-timeout 120 sbus
container_id="$(docker compose --project-name "$project_name" --env-file .env.example -f compose.yaml ps -q sbus)"
test -n "$container_id"
...
echo "sbus-release-package: PASS"
