...
class ReleasePackageTest(unittest.TestCase):
    ...
    def test_runtime_is_nonroot_and_uses_installed_distribution(self):
        ...
        self.assertIn('/app/bin/payment-sbus', DOCKERFILE)
        self.assertNotRegex(DOCKERFILE, r"payment-sbus-[0-9].*\.jar")

    def test_healthcheck_adds_no_runtime_package(self):
        self.assertIn("HEALTHCHECK", DOCKERFILE)
        self.assertNotRegex(DOCKERFILE, r"\b(?:apk|apt-get)\b")

    def test_compose_owns_only_sbus_and_external_network(self):
        services = COMPOSE.split("services:\n", 1)[1].split("\nnetworks:\n", 1)[0]
        self.assertEqual(["sbus"], re.findall(r"^  ([a-z][a-z0-9-]*):$", services, re.MULTILINE))
    ...
    def test_ci_covers_build_integration_image_sbom_scan_and_docs(self):
        for marker in ("./gradlew test --no-daemon", "-PwithIT", "docker buildx build",
                       "--build-context contracts-repository", "sbom-action", "trivy-action",
    ...
    def test_owned_runbooks_cover_retry_dlq_and_rollback(self):
        runbooks = (ROOT / "ops/runbooks/README.md").read_text(encoding="utf-8")
        self.assertIn("retry-backlog.md", runbooks)
        self.assertIn("dlq-unconfirmed.md", runbooks)
