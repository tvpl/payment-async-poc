...
REQUIRED = {
    ...
    "docs/adr/0001-stream-retention-and-wakeup-protocol.md",
    "ops/runbooks/README.md", "ops/runbooks/backlog-retention.md",
    "ops/runbooks/poison-dlq.md", "ops/runbooks/worker-outage-recovery.md",
    "ops/runbooks/release-rollback.md",
}
LINK = re.compile(r"(?<!!)\[[^]]+\]\(([^)]+)\)")
...
REQUIRED_MARKERS = {
    ...
    "docs/contracts.md": ("POST /jobs", "GET /jobs/{jobId}", "Idempotency-Key"),
    "docs/operations.md": ("## Pré-requisitos", "## Startup e readiness", "## Rollback"),
...
FORBIDDEN = ("project(':", 'project(":', "docker compose down -v",
             "payment-contracts/src", "feature-control/src", "payment-api/src", "payment-sbus/src",
             "payment-core-mock/src")
...
def validate(root: Path) -> list[str]:
    ...
    adr = root / "docs/adr/0001-stream-retention-and-wakeup-protocol.md"
    if adr.is_file():
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print(f"async-redis-docs: PASS ({len(REQUIRED)} required documents, links, claims, ADR)")
    return 0
